...
function WalletGraphView(props) {
  ...
  var currentWallet = props.currentWallet;
  if (!_.isEmpty(currentWallet) || true) {
    height = 0;
    var yCoordGap = height / txnData.length;
    const xCoordGap = 160;
    var yCoord = yCoordGap;
    var xCoord = xCoordGap;

    data.nodes.push({
      id: currentWallet.id, // "sourceNode", // String, unique and required
      label:
    ...
    // fromNodes.push(currentWallet.id);
    ...
    while (!_.isEmpty(queue)) {
      ...
      for (const txn of txnData) {
        ...
        height += 5;
        // yCoordGap = height / txnData.length;
        yCoordGap = 0;

        if (
          txn.fromAddress == headAddr &&
          !doneTxn.includes(txn.id) &&
          ...
          console.log("TRANSACTION>>>", headAddr);
          if (fromNodes.includes(txn.fromAddress)) {
            data.nodes.push({
              id: txn.toAddress,
              x: xCoord - xCoordGap,
          ...
          fromNodes.push(txn.fromAddress);
          ...
          yCoord += yCoordGap;
          xCoord += xCoordGap;
        } else if (txn.fromAddress == headAddr && !doneTxn.includes(txn.id)) {
          queue.push(txn.toAddress);
          doneTxn.push(txn.id);
